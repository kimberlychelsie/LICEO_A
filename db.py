            ...
            try:
                ...
                if ii_cols:
                    ...
                    if 'is_set_piece' not in ii_cols:
                        cur.execute("ALTER TABLE inventory_items ADD COLUMN is_set_piece BOOLEAN DEFAULT FALSE")
                    conn.commit()
                # ── Auto-seed default uniform catalog sets & pieces per branch if missing ──
                try:
                    cur.execute("SELECT branch_id FROM branches WHERE status = 'active'")
                    active_branches = [r[0] for r in cur.fetchall()]
                    if not active_branches:
                        active_branches = [1]  # fallback

                    default_uniform_sets = [
                        {
                            "item_name": "Pre-Elementary Boys Set",
                            "grade_level": "Nursery, Kinder, Grade 1-3",
                            "price": 1000.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/PRE_ELEM_BOYS_SET.jpg",
                            "pieces": [
                                {"item_name": "Polo", "price": 450.00},
                                {"item_name": "Pants", "price": 460.00}
                            ]
                        },
                        {
                            "item_name": "Pre-Elementary Girls Set",
                            "grade_level": "Nursery, Kinder, Grade 1-6",
                            "price": 1000.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/PRE_ELEM_GIRLS_SET.jpg",
                            "pieces": [
                                {"item_name": "Blouse", "price": 450.00},
                                {"item_name": "Skirt", "price": 460.00}
                            ]
                        },
                        {
                            "item_name": "Elementary G4-6 Boys Set",
                            "grade_level": "Grade 4-6",
                            "price": 1000.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/ELEM_G4to6_BOYS_SET.jpg",
                            "pieces": [
                                {"item_name": "Polo", "price": 450.00},
                                {"item_name": "Pants", "price": 460.00}
                            ]
                        ...
                        {
                            "item_name": "JHS Boys Uniform Set",
                            "grade_level": "Grade 7-10",
                            "price": 1200.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/JHS_BOYS_SET.jpg",
                            "pieces": [
                                {"item_name": "Polo", "price": 500.00},
                                {"item_name": "Pants", "price": 520.00}
                            ]
                        },
                        {
                            "item_name": "JHS Girls Uniform Set",
                            "grade_level": "Grade 7-10",
                            "price": 1250.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/JHS_GIRLS_SET.jpg",
                            "pieces": [
                                {"item_name": "Blouse", "price": 500.00},
                                {"item_name": "Skirt", "price": 550.00}
                            ]
                        },
                        {
                            "item_name": "SHS Boys Uniform Set",
                            "grade_level": "Grade 11-12",
                            "price": 1300.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/SHS_BOYS_SET.jpg",
                            "pieces": [
                                {"item_name": "Polo", "price": 550.00},
                                {"item_name": "Pants", "price": 580.00}
                            ]
                        },
                        {
                            "item_name": "SHS Girls Uniform Set",
                            "grade_level": "Grade 11-12",
                            "price": 1350.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/SHS_GIRLS_SET.jpg",
                            "pieces": [
                                {"item_name": "Blouse", "price": 550.00},
                                {"item_name": "Skirt", "price": 600.00}
                            ]
                        },
                        {
                            "item_name": "PE Uniform",
                            "grade_level": "All Grades",
                            "price": 850.00,
                            "size_label": "XS, S, M, L, XL, XXL, XXXL",
                            "image_url": "/static/img/PE_SET.jpg",
                            "pieces": [
                                {"item_name": "PE Shirt", "price": 400.00},
                                {"item_name": "PE Pants", "price": 450.00}
                            ]
                        }
                    ]

                    for b_id in active_branches:
                        cur.execute("SELECT COUNT(*) FROM inventory_items WHERE branch_id = %s AND UPPER(category) = 'UNIFORM'", (b_id,))
                        if cur.fetchone()[0] == 0:
                            logger.info(f"Seeding default uniform catalog for branch {b_id}...")
                            for set_data in default_uniform_sets:
                                cur.execute("""
                                    ...
                                    VALUES (%s, 'UNIFORM', %s, %s, %s, %s, %s, TRUE, NULL, FALSE, 20.00)
                                    RETURNING item_id
                                """, (
                                    b_id, set_data["item_name"], set_data["grade_level"], set_data["price"],
                                    set_data["size_label"], set_data["image_url"]
                                ))
                                    ...
                                    cur.execute("""
                                        INSERT INTO inventory_items
                                          (branch_id, category, item_name, grade_level, price, size_label,
                                           is_active, parent_item_id, is_set_piece, size_price_step)
                                        VALUES (%s, 'UNIFORM', %s, %s, %s, %s, TRUE, %s, TRUE, 20.00)
                                    """, (
                                        b_id, piece["item_name"], set_data["grade_level"], piece["price"],
                                        set_data["size_label"], parent_id
                                    ))
                    conn.commit()
                except Exception as e:
                    logger.warning(f"Could not auto-seed uniform catalog: {e}")
                    conn.rollback()
