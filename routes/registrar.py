...
def create_parent_account(enrollment_id):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE (role='parent' OR user_roles ILIKE '%%parent%%') AND branch_id=%s AND username ILIKE %s
    ...
    try:
        ...
        if tab == "students":
            ...
            cursor.execute("""
                ...
                WHERE u.branch_id = %s AND (u.role = 'teacher' OR u.user_roles ILIKE '%%teacher%%')
...
def list_and_add_schedules():
    ...
    if request.method == "POST":
        ...
        if not (time(6,0) <= start_t <= time(18,0)) or not (time(6,0) <= end_t <= time(18,0)):
        ...
        cursor.close(); db.close()
        return redirect(url_for("registrar.list_and_add_schedules", grade=added_grade, section_id=section_id))
...
def registrar_delete_archived_teacher(user_id):
    ...
    try:
        ...
        cursor.execute("DELETE FROM teacher_grade_levels WHERE teacher_id = %s", (user_id,))
        cursor.execute(
            "DELETE FROM users WHERE user_id = %s AND branch_id = %s AND (role = 'teacher' OR user_roles ILIKE '%%teacher%%') AND COALESCE(is_archived, FALSE) = TRUE",
            (user_id, branch_id),
        )
        db.commit()
...
# ══════════════════════════════════════════════════════════════
# GRADE REVIEW — REGISTRAR
# ══════════════════════════════════════════════════════════════

@registrar_bp.route("/registrar/grade-review")
def registrar_grade_review():
    if session.get("role") != "registrar":
        return redirect("/")
    branch_id = session.get("branch_id")
    ...
    try:
        ...
        query = """
            SELECT r.id, r.section_id, r.subject_id, r.grading_period, r.status,
                   r.submitted_at, r.rejection_remarks,
        ...
        return render_template("registrar_grade_review.html",
            grade_levels=grade_levels,
            sections=sections,
...
@registrar_bp.route("/registrar/grade-review/<int:req_id>/approve", methods=["POST"])
def registrar_approve_grade_request(req_id):
    if session.get("role") != "registrar":
        return redirect("/")
    ...
    try:
        cur.execute("""
            ...
            SET status = 'pending_admin',
                registrar_approved_by = %s,
                registrar_approved_at = NOW(),
                rejection_remarks = NULL
            WHERE id = %s AND branch_id = %s AND status = 'pending_registrar'
        """, (user_id, req_id, branch_id))
        db.commit()
        flash("Grade submission approved and forwarded to Branch Admin for final approval.", "success")
    except Exception as e:
        db.rollback()
...
@registrar_bp.route("/registrar/grade-review/<int:req_id>/reject", methods=["POST"])
def registrar_reject_grade_request(req_id):
    if session.get("role") != "registrar":
        return redirect("/")
    user_id = session.get("user_id")
    branch_id = session.get("branch_id")
