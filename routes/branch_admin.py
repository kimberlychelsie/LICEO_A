   ...
   try:
       ...
       if request.method == "POST":
           ...
           """, (
               ...
               active_year_id
                   ...
                   already_here = int(
                       cursor.fetchone()["already_here"] or 0
                   )


                   slots_needed = len(enrollment_ids) - already_here


                   available_slots = (
                       int(sec_info["capacity"] or 0)
                       - int(sec_info["current_count"] or 0)
                   )
                   ...
                   if slots_needed > available_slots:
                       return jsonify({
                           "success": False,
                           "message": (
                               f"Section does not have enough available slots. "
                               f"Available: {available_slots}, "
                               f"Needed: {slots_needed}."
                           )
                       }), 400
       ...
       """, (
           ...
           active_year_id
               ...
               if len(valid_ids) != len(set(enrollment_ids)):
                   return jsonify({
                       "success": False,
                       "message": "One or more student enrollments were not found."
                   }), 400
           ...
           active_year_id
               ...
               return jsonify({
                   ...
                   "message": (
                       ...
                       else f"{len(enrollment_ids)} students updated successfully."
                   )
               })
...
@branch_admin_bp.route("/branch-admin/grade-approval/<int:req_id>/class-record")
def branch_admin_view_class_record(req_id):
    if session.get("role") != "branch_admin":
        return redirect("/")
    branch_id = session.get("branch_id")
    ...
    try:
        ...
        if not req_info:
            ...
            return redirect(url_for("branch_admin.branch_admin_grade_approval"))
        ...
        return render_template("branch_admin_class_record_approval.html",
            req_info=req_info,
            weights=weights,
            records=records
        )
